...
class LatentAdversarialTrainingDataCollator:
    ...
    def __call__(self, batch):
        ...
        for i in range(B):
            ...
            adv_prompt_lengths.append(
                batch[i]["prompt_length"] + batch[i]["adv_length"]
            )
            def_prompt_lengths.append(
                batch[i]["prompt_length"] + batch[i]["def_length"]
            )
        ...
        for i in range(B):
            adv_prompt_tokens[i] = torch.tensor(
                batch[i]["prompt_tokens"]
                + batch[i]["adv_tokens"]
                + [self.pad_token_id] * (pad_length - adv_prompt_lengths[i])
            )
            def_prompt_tokens[i] = torch.tensor(
                batch[i]["prompt_tokens"]
                + batch[i]["def_tokens"]
                + [self.pad_token_id] * (pad_length - def_prompt_lengths[i])
            )
        ...
        if self.truncate_length is not None:
            if any(
                [
                    prompt_length > self.truncate_length
                    for prompt_length in prompt_lengths
                ]
            ):
                print(
                    f"WARNING: Prompt length (at least one of {prompt_lengths}) is less than truncate length ({self.truncate_length})"
                )
...
def apply_chat_formatting(
    ...
    if use_tokenizer_template:
        ...
        if system_prompt is not None:
            prompt_str = custom_prompt_template.format(
...
def process_generic_chat_dataset(
    ...
    **dataset_kwargs,
    ...
    assert not (system_prompt is not None and system_prompt_column is not None), (
    ...
    def preprocess_example_batch(examples):
        for i in range(len(examples["prompt"])):
            ...
            if num_adv_words is None:
                ...
                examples["adv_completion"][i] = " ".join(
                    adv_completion.split(" ")[:num_adv_words]
                )
    ...
    def tokenize_batch(examples):
        ...
        examples["adv_tokens"] = tokenizer(
            examples["adv_completion"], add_special_tokens=False
        ).input_ids
        examples["def_tokens"] = tokenizer(
            examples["def_completion"], add_special_tokens=False
        ).input_ids
        return examples
    ...
    if filter_len is not None:
        ...
        dataset = dataset.filter(
            lambda x: len(x["prompt_tokens"])
            + max(len(x["adv_tokens"]), len(x["def_tokens"]))
            <= filter_len
        )
        end_len = len(dataset)
        print(
...
def process_pretokenized_dataset(
    ...
    dataset = dataset.map(
        lambda examples: {
            "adv_tokens": examples["prompt_tokens"],
            "def_tokens": examples["prompt_tokens"],
        },
        batched=True,
    )

    # adv and def shouldn't both be None
    assert adv_labels_column is not None or def_labels_column is not None, (
        "At least one of adv_labels_column or def_labels_column should almost certainly be specified, error for now"
    )
    ...
    def create_default_indices(examples, adv=True):
        ...
        for i in range(len(examples["prompt_tokens"])):
            ...
            indices = list(
                range(len(labels))
            )  # next_token_prediction at last token is not a label
            if adv:
    ...
    if adv_labels_column is None:
        assert adv_labels_indices_column is None, (
            f"If adv_labels_column is None, adv_labels_indices_column must also be None, instead got {adv_labels_indices_column}"
        )
        dataset = dataset.map(
            lambda examples: create_default_indices(examples, adv=True), batched=True
        )

    else:
        if adv_labels_column != "adv_labels":
            dataset = dataset.rename_column(adv_labels_column, "adv_labels")
        assert adv_labels_indices_column is not None, (
            f"If adv_labels_column is not None, adv_labels_indices_column must also be specified, instead got {adv_labels_indices_column}"
        )
    ...
    if def_labels_column is None:
        assert def_labels_indices_column is None, (
            f"If def_labels_column is None, def_labels_indices_column must also be None, instead got {def_labels_indices_column}"
        )
        dataset = dataset.map(
            lambda examples: create_default_indices(examples, adv=False), batched=True
        )

    else:
        if def_labels_column != "def_labels":
            dataset = dataset.rename_column(def_labels_column, "def_labels")
        assert def_labels_indices_column is not None, (
            f"If def_labels_column is not None, def_labels_indices_column must also be specified, instead got {def_labels_indices_column}"
        )
    ...
    def check_labels_lengths(examples):
        ...
        for i in range(len(examples["adv_labels"])):
            assert len(examples["adv_labels"][i]) == len(examples["adv_indices"][i]), (
                f"Length of adv_labels and adv_indices should be the same, got {len(examples['adv_labels'][i])} and {len(examples['adv_indices'][i])}"
            )
            assert len(examples["def_labels"][i]) == len(examples["def_indices"][i]), (
                f"Length of def_labels and def_indices should be the same, got {len(examples['def_labels'][i])} and {len(examples['def_indices'][i])}"
            )
...
def load_targeted_wmdp_data(
    ...
    forget_corpora: List[str] = ["wikitext", "wikitext"],
...
def make_targeted_wmdp_dataloader(
    ...
    dataloader = DataLoader(
        ...
        collate_fn=WMDPLATTargetedDataCollator(
            tokenizer, truncate_length=data_truncate_length
        ),
        drop_last=True,
    )
    return dataloader
...
def make_untargeted_wmdp_dataloaders(
    ...
    if sft:
        return DataLoader(
            ...
            collate_fn=WMDPLATDataCollator(
                tokenizer, truncate_length=data_truncate_length
            ),
            drop_last=True,
        )
...
class PretokenizedLatentAdversarialTrainingDataCollator:
    ...
    def __call__(self, batch):
        ...
        for i in range(B):
            adv_prompt_tokens[i] = torch.tensor(
                batch[i]["prompt_tokens"]
                + [self.pad_token_id] * (pad_length - prompt_lengths[i] + 1)
            )
            def_prompt_tokens[i] = torch.tensor(
                batch[i]["prompt_tokens"]
                + [self.pad_token_id] * (pad_length - prompt_lengths[i] + 1)
            ...
            for adv_index in batch[i]["adv_indices"]:
                ...
                if adv_index + 1 >= prompt_lengths[i]:
                    print(
                        f"adv_index {adv_index} is within 1 of prompt_length {prompt_lengths[i]}, will not check labels for this index"
                    )
                    continue
                adv_labels_mask[i, adv_index + 1] = (
                    True  # + 1 because indexing logits by final_logits = logits[:, :-1][towards_labels_mask[:, 1:]], need to shift by 1
                )

            for def_index in batch[i]["def_indices"]:
                assert def_index + 1 < prompt_lengths[i], (
                    f"def_index {def_index} is within 1 of prompt_length {prompt_lengths[i]}"
                )
...
class WMDPLATTargetedDataCollator:
    ...
    def __call__(self, batch: List[str]):
        B = len(batch)
        tokenized_def_inputs = [
            self.tokenizer(example["def_tokens"])["input_ids"] for example in batch
        ]
        tokenized_adv_inputs = [
            self.tokenizer(example["adv_tokens"])["input_ids"] for example in batch
        ]
        ...
        for i, (def_inputs, adv_inputs) in enumerate(
            zip(tokenized_def_inputs, tokenized_adv_inputs)
        ):
            def_tokens[i] = torch.tensor(
                def_inputs + [self.pad_token_id] * (pad_length - def_lengths[i]),
                dtype=torch.long,
            )
            adv_tokens[i] = torch.tensor(
                adv_inputs + [self.pad_token_id] * (pad_length - adv_lengths[i]),
                dtype=torch.long,
            )
    ...
    def __call__(self, batch: List[str]):
        ...
        for i, example in enumerate(tokenized_inputs):
            ...
            tokens[i] = torch.tensor(
                example + [self.pad_token_id] * (pad_length - length), dtype=torch.long
            )
            adv_labels_mask[i, :length] = True
            def_labels_mask[i, :length] = True

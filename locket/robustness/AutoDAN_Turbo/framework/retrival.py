...
class Retrieval:
    ...
    def embed(self, text):
        ...
        try:
            ...
            self.logger.error(
                f"Failed to convert embedding to np.array: {e}", exc_info=True
            )
            return None
    ...
    def pop(self, library, query, k=5):
        ...
        self.logger.info("Searching for similar strategies")
        ...
        for s_name, s_info in library.items():
            ...
            for i in range(len(emb_list)):
                ...
                reverse_map.append(
                    s_name
                )  # So we know which strategy this embedding belongs to
        ...
        distances, indices = index.search(
            query_embedding.reshape(1, dim), num_to_retrieve
        )
        ...
        for final_s_name, final_s_info in retrieved_strategies.items():
            if final_s_info["Score"] >= 5:
                new_dict = {
                    key: value for key, value in final_s_info.items() if key != "Score"
                }
